...
class WhisperAudioProcessor @Inject constructor(
    ...
    suspend fun preProcessing(
        ...
        inferenceAudioChannel: Channel<FloatArray>,
    ) {
        runCatching {
            val stepSize = extractedAudioInfo.mediaInfo.sampleRate * 30 * Float.SIZE_BYTES

            if (::samplingAudioList.isInitialized.not())
                samplingAudioList = ByteArray(stepSize)

            if (lastProducedAudioIndex + extractedAudioInfo.audioSize > stepSize) {
